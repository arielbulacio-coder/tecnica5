        ...
        <div className="drawing-container">
            ...
            <div className="drawing-grid">
                            ...
                            <div className="canvas-wrapper">
                                <svg viewBox="0 0 100 100" className="construction-svg">
                                    ...
                                    {currentConst.steps.slice(0, step + 1).map((s, idx) => (
                                        <g key={idx} className={`step-group ${idx === step ? 'active-step' : 'past-step'}`}>
                                            {s.viz(idx)}
                                        </g>
                                    ))}
                                </svg>
                ...
                <section className="drawing-card level-advanced">
                    <h2>Tips de Precisión Técnica</h2>
                    <div className="tips-grid">
                        <div className="tip-item">
                            <strong>Afilado de Mina:</strong> Mantén la mina del compás afilada en forma de "bisel" para trazos finos y precisos.
                        </div>
                        <div className="tip-item">
                            <strong>Puntos de Apoyo:</strong> Presiona levemente sobre el centro para no perforar la hoja, pero mantén firme la aguja.
                        </div>
                        <div className="tip-item">
                            <strong>Empalmes:</strong> Los arcos de círculo deben tocarse tangencialmente, sin "escalones" visibles entre ellos.
                        </div>
                    </div>
                </section>
            </div>
            ...
            <style>{`
                ...
                .module-info { display: flex; flex-direction: column; justify-content: center; }
                ...
                .ctrl-btn.main { background: var(--primary-color); color: #000; font-weight: bold; border: none; }
                .ctrl-btn.reset { color: var(--text-dim); }
                .ctrl-btn:hover:not(:disabled) { transform: scale(1.05); }
                .canvas-wrapper { position: relative; background: #fff; padding: 1rem; border-radius: 10px; box-shadow: inset 0 0 20px rgba(0,0,0,0.1); }
                .construction-svg { width: 100%; height: auto; }
                .active-step { opacity: 1; stroke-width: 2px; }
                .past-step { opacity: 0.4; }
                .tips-grid { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 1rem; margin-top: 1rem; font-size: 0.85rem; }
                .tip-item { padding: 1rem; background: rgba(255,255,255,0.05); border-radius: 8px; border-top: 2px solid var(--secondary-color); }
